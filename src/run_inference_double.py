...
def main():
    ...
    for file in tqdm(test_files):
        ...
        if (not args.no_resize) and (h != args.img_height or w != args.img_width):
            img = np.array(Image.fromarray(img).resize((args.img_height, args.img_width)))
        ...
        if args.output_disp:
            ...
            disp = np.dstack((img, disp))
            imsave(output_dir/'{}_disp_dbl{}'.format(file_name, file_ext), np.transpose(disp, (1,2,0)))
        if args.output_depth:
            depth = 1/output
            depth = (255*tensor2array(depth, max_value=10, colormap='rainbow')).astype(np.uint8)
            depth = np.dstack((img, depth))
            imsave(output_dir/'{}_depth_dbl{}'.format(file_name, file_ext), np.transpose(depth, (1,2,0)))
