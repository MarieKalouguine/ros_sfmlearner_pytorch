...
class KittiRawLoader(object):
    def __init__(self,
                 ...
                 get_pose=False,
                 depth_size_ratio=1):
        dir_path = Path(__file__).realpath().dirname()  #directory containing the current file
        test_scene_file = dir_path/'test_scenes.txt'

        self.from_speed = static_frames_file is None
        if static_frames_file is not None:  #if the user specified frames to discard
            static_frames_file = Path(static_frames_file)
            self.collect_static_frames(static_frames_file)  # fill self.static_frames with the specified images
        ...
        self.collect_train_folders()    #loading data in self.scenes
    ...
    def collect_scenes(self, drive):
        ...
        for c in self.cam_ids:
            ...
            for n, f in enumerate(oxts):    #n is the iterator variable
                metadata = np.genfromtxt(f)
                speed = metadata[8:11]
                scene_data['speed'].append(speed)
                scene_data['frame_id'].append('{:010d}'.format(n))  #the 10 first digits of n
                lat = metadata[0]
